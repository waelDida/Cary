...
class DeleteCareSeekerData : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_delete_care_seeker_data, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val toDoApp = (requireContext().applicationContext) as ToDoApplication
        val application = requireActivity().application
        val seekerDataRepository = toDoApp.seekerDataRepository
        val inboxDataRepository = toDoApp.inboxDataRepository
        val reviewsDataRepository = toDoApp.reviewsDataRepository
        val viewModelFactory = DeleteSeekerVMFactory(seekerDataRepository,inboxDataRepository,reviewsDataRepository,application)
        val viewModel = ViewModelProvider(this,viewModelFactory).get(DeleteSeekerDataVM::class.java)
